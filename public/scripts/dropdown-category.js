...
document.addEventListener("DOMContentLoaded", function() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Check for specific error before checking response.ok
      if (data.error && data.error.includes('Item_Already_Exists')) {
        // Trigger jQuery to inform the user
        console.log("$('.error-container')");
        $('.error-container').text('Item already exists in your to-do list.').slideDown();
        // Clear displayInformation
        document.getElementById("displayInformation").innerHTML = '';

        //STRETCH: Add the info of the existed item in the list
        return;
      }
...
// Function to display information on the screen
  function displayInformationOnScreen(info) {
    ...
    console.log("In function displayInformation: ", info);
